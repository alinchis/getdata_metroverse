...
const OUTPUT_FILE = "./data/clusterDensityRescale.json";
const DELAY_MS = 10;
...
const QUERY = `
query GetClusterDensityRescale(
  $cityId: Int!
  $peerGroup: String
  $partnerCityIds: [Int]
  $clusterLevel: Int = 3
...
async function fetchCityData(cityId) {
  ...
  return json.data.clusterDensityRescale;
}
...
async function main() {
  ...
  for (let i = 0; i < cityIds.length; i++) {
    ...
    console.log(`[${i + 1}/${cityIds.length}] Fetching clusterDensityRescale for city ${cityId}...`);
